...
static jint nativeTtControl(JNIEnv* env, jclass clazz, jint teletextEvent, jint magazine, jint page, jint regionId) {
    if (getJniContext()->mSubContext != nullptr) {
        getJniContext()->mSubContext->ttControl(teletextEvent, magazine, page, regionId, -1);
    } else {
        ALOGE("Subtitle Connection not established");
    }
    return 0;
