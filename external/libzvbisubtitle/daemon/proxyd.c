...
** - query driver for new norm: if successful, this overrides information
...
static vbi_bool vbi_proxyd_start_acq_thread( int dev_idx )
{
   ...
   if (pipe(pipe_fds) == 0)
   {
      ...
      {
         dprintf(DBG_MSG, "acquisition thread started: "
...
** - if the token is still in possession of a different client the request will
...
** - note: consistency checks were already done by the I/O handler
         ...
         if (req->state == REQ_STATE_WAIT_CON_REQ)
         {
            if (pBody->connect_req.magics.protocol_compat_version == VBIPROXY_COMPAT_VERSION)
            {
                                                     ...
                                                     req->msg_buf.body.connect_rej.errorstr) )
               {
                  ...
                  {  /* acquisition not running: if the request is still considered successful
...
static void vbi_proxyd_main_loop( void )
{
   ...
   while (proxy.should_exit == FALSE)
   {
      ...
      /* wait for new clients, client messages or VBI device data (indefinitely) */
