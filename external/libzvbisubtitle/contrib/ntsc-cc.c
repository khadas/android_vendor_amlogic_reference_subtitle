...
static int CCdecode(int data)
{
	...
	else if ((b1&0x10) && (b2>0x1F) && (data != lastcode)) //codes are always transmitted twice (apparently not, ignore the second occurrence)
	{
		ccmode=((b1>>3)&1)+1;
...
//					fprintf (cc_fp[cur_ch[field]], "ccmode %d cmd %02x\n",ccmode,b2);
					...
					switch (b2)
					{
						...
						/* these codes are insignificant if we're ignoring positioning */
