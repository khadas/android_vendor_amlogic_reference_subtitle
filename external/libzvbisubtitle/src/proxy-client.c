   ...
   switch (pHead->type)
   {
      case MSG_TYPE_CONNECT_CNF:
              ...
              (memcmp(pBody->connect_cnf.magics.protocol_magic, VBIPROXY_MAGIC_STR, VBIPROXY_MAGIC_LEN) == 0) )
         {
            ...
            {  /* endian type does not match -> convert "endianness" of all msg elements > 1 byte */
...
**   does not return until a reply is available or a timeout occurred (in which
                                      ...
                                      struct timeval * p_timeout )
{
   ...
   if (ret > 0)
   {
      ...
      /* process the message - frees the buffer if necessary */
      if (proxy_client_take_message(vpc) == FALSE)
         goto failure;
   }
 ...
 * indications can be suppressed with VBI_PROXY_CLIENT_NO_STATUS_IND
 ...
 * The function also will fail with errno @c EBUSY if the client doesn't
 ...
 * caller, i.e. accessed neither for read nor write.  Returns @c NULL
 * upon error (i.e. if the client is not connected to the daemon)
 *
 * @since 0.2.9
 */
