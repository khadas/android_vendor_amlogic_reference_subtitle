...
#define dump_log(...) __android_log_print(ANDROID_LOG_INFO, "SCTE_DUMP", __VA_ARGS__)

#define AM_DEBUG(_level,_fmt...) scte_log(_fmt)
...
static uint32_t am_scte_get_video_pts(void* media_sync)
{
...
static void* scte27_thread(void *arg)
{
	...
	if (media_sync != NULL) {
		AM_DEBUG(0, " am_scte27.c destroy the media sync handler");
...
int decode_message_body(AM_SCTE27_Parser_t *parser, const uint8_t *buf, int size)
{
	...
	if (parser->para.lang_cb && lang_valid == 0 && !parser->para.hasReportLang) {
		parser->para.lang_cb(parser, sub_node.lang, 4);
		parser->para.hasReportLang = AM_TRUE;
	}
	...
	{
		scte_log("update size last w x h:(%d x %d), now w x h:(%d x %d)",
