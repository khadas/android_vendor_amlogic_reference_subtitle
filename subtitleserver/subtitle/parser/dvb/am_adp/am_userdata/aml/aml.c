...
static userdata_type aml_check_userdata_format (uint8_t *buf, int vfmt, int len)
{
	...
	if (vfmt == 2)
	{
		...
		{
			AM_DEBUG(AM_DEBUG_LEVEL,"CC format is h264_or_h265_cc_type");
			return H264_CC_TYPE;
		}
...
static void aml_write_userdata(AM_USERDATA_Device_t *dev, uint8_t *buffer, int buffer_len, int64_t
	pts, int pts_valid, int64_t duration)
{
...
static void aml_add_cc_data(AM_USERDATA_Device_t *dev, int poc, int type, uint8_t *p, int len, int64_t
	pts, int pts_valid, int64_t duration)
{
	AM_UDDrvData *ud = dev->drv_data;
	AM_CCData **pcc, *cc;
...
len, int64_t pts, int pts_valid, int64_t duration)
{
	AM_UDDrvData *ud = dev->drv_data;
#if 0
	int i;
...
static void aml_h264_userdata_package(AM_USERDATA_Device_t *dev, int poc, int type, uint8_t *p, int len, int64_t
	pts, int pts_valid, int64_t duration, void* media_sync)
{
	AM_UDDrvData *ud = dev->drv_data;
	AM_CCData *cc;
...
static int aml_process_h264_userdata(AM_USERDATA_Device_t *dev, uint8_t *data, int len, struct userdata_meta_info_t* meta_info, void* media_sync)
{
	AM_UDDrvData *ud = dev->drv_data;
	int fd = ud->fd;
	uint8_t *pd = data;
